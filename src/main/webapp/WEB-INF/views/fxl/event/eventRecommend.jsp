...
$(document).ready(function() {
	...
	var eventTable = $("table#eventTable").DataTable({
		...
		"columnDefs" : [{
			...
			"targets" : [5],
			"render" : function(data, type, full) {
				return '<button id="release" mId="' + full.DT_RowId + '" data-eventtitle="' + full.ftitle + '" type="button" class="btn btn-success btn-xs">设置标签&推荐分</button>';
			}
		}]
	});
	...
	/* eventPersonalizedForm.validationEngine({
		maxErrorsPerField: 1,
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h3 class="modal-title" id="myModalLabel">请选择您想要的活动标签</h3>
			</div>
			<div class="modal-body">
				...
				<form id="eventPersonalizedForm" action="${ctx}/fxl/event/addPersonalizedTag" method="post" class="form-inline" role="form">
					<input id="eventId" name="eventId" type="hidden">
					
				<!-- 选择活动位栏目 -->
				<div class="panel panel-success">
					<div class="panel-heading">
							<strong>选择活动标签(最多5个)</strong>
					</div>
					   <div class="panel-body">
						   <div class="row">
							  <div class="form-group"><label for="ftag">活动标签：</label>
		                         <select id="ftag" name="ftag" class="form-control" multiple="multiple" style="width: 550px;">
			                        <c:forEach var="tagItem" items="${tagMap}"> 
			                                <option value="${tagItem.value}">${tagItem.value}</option>
			                         </c:forEach>
		                          </select>
                               </div>
