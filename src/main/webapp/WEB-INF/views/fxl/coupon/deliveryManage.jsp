...
$(document).ready(function() {
	...
	var couponTable = $("table#couponTable").DataTable({
		...
		"columnDefs" : [{
			...
			"render" : function(data, type, full) {
				var refString = '';
				refString =  '<button id="offsale" mId="' + full.DT_RowId + '" type="button" class="btn btn-success btn-xs">查看</button>';
				if(data.status == 30){
					refString += '<button id="issue" mId="' + full.DT_RowId + '" type="button" class="btn btn-info btn-xs">发放</button>';
				}
				return refString;
			}
		}]
	...
	$("#couponTable").delegate("button[id=issue]", "click", function(){
		...
		params['status']=90;//审核通过
		dialog({
			fixed: true,
		    title: '操作提示',
		    content: '您确定要将优惠券发放给用户吗？',
		    okValue: '发放成功',
		    ok: function () {
		    	$.post("${ctx}/fxl/coupon/issueDelivery",params, function(data) {
					if(data.success){
...
<div class="row">
  <div class="col-md-8"><h3>定向投放活动维护</h3></div>
...
<div class="row">
  <div class="col-md-2"><h4>定向投放列表</h4></div>
  <div class="col-md-10"><p class="text-right"><button id="createEventBtn" type="button" class="btn btn-primary"><span class="glyphicon glyphicon-plus-sign"></span> 创建定向投放活动</button>
...
<div id="selectDiv" class="text-center">
	<form id="searchForm" action="${ctx}/fxl/coupon/getDeliveryList" method="post" class="form-inline" role="form">
		<div class="form-group"><label for="s_ftitle">定向投放活动标题：</label>
