...
$(document).ready(function() {
	...
	var coupon_panel_history = $("table#coupon_panel_history").DataTable({
		...
		"columnDefs" : [{
			...
			"render" : function(data, type, full) {
				...
				full.limitation + '" validDays="' + full.validDays +'" amount="' + full.amount + '" discount="' + full.discount + '" coupondesc="' + 
				full.coupondesc + '"/>'
			}
		}]
	});
    ...
    saveCouponForm.on("submit", function(event){
		...
		if(form.validationEngine("validate") && form.data("running") != "ok"){
			...
			$.post(form.attr("action"), $.param($.merge(form.serializeArray(),[{name:"fCouponType", value:20}]),true), function(data){
				if(data.success){
					toastr.success(data.msg);
					window.location.href = "${ctx}/fxl/coupon/toissueCreateMain";
				}else{
    ...
    $("#coupon_grid_new").delegate("button[id=del]", "click", function(){
  	...
  	$('#nextCouponNew').click(function(){
        ...
        window.location.href = "${ctx}/fxl/coupon/toissueDeliveryCreate?ids="+ids;
    });
...
<div class="text">
	...
	<div id="createEventAForm"  class="form-inline" >
				...
				<div class="col-md-6" style="height: 500px;">
					...
					<div class="table-responsive">
					    ...
					    <div class="form-group has-error"><label for="fuseStartTime">可用有效期：</label>
								<div id="fuseTimeDiv" class="input-daterange input-group date" style="width:300px;">
									<input type="text" class="form-control " id="fuseStartTime" name="fuseStartTime" style="cursor: pointer;"><span class="input-group-addon"><fmt:message key="fxl.common.to" /></span>
									<input type="text" class="form-control " id="fuseEndTime" name="fuseEndTime" style="cursor: pointer;">
								</div>
								<div class="input-group">
								<div class="input-group-addon">或者相对</div>
									<input type="text" id="fvalidDays" name="fvalidDays" class="form-control validate[custom[number],min[0],max[999]]" size="5"><div class="input-group-addon">天</div>
							</div>
						</div>
		...
		<div class="row coupon-new">
			<span class="coupon-tip">即将发放的券</span><br>
