...
$(document).ready(function() {
	...
	var reconciliationTable = $("table#reconciliationTable").DataTable({
		...
		"serverSide": true,
		"ajax": {
		    "url": "${ctx}/fxl/report/getFinanceReconList",
 		    "type": "POST",
 		    "data": function (data) {
		...
		{	"title" : "ID",
			...
			"title" : '<center>订单数量</center>',
			"data" : "fcount",
			"className": "text-center",
			"width" : "100px",
			"orderable" : false
		}, {
			"title" : '<center>订单总金额</center>',
			"data" : "ftotal",
			"className": "text-center",
			"width" : "40px",
			"orderable" : false
		}, {
			"title" : '<center>使用优惠券金额</center>',
			"data" : "fchangeAmount",
			"className": "text-center",
			"width" : "40px",
			"orderable" : false
		}, {
			"title" : '<center>通道费率</center>',
			"data" : "rate",
			"className": "text-center",
			"width" : "40px",
			"orderable" : false
		}, {
			"title" : '<center>应到帐金额</center>',
			"data" : "ftotalPrice",
			"className": "text-center",
			"width" : "50px",
			"orderable" : false
		...
		if(exportExcelBtn.data("running") != "ok"){
			...
			dialog({
			    ...
			    ok: function () {
			    	...
			    	$.post("${ctx}/fxl/report/createFinanceReconExcel" , searchForm.serializeArray(), function(data) {
						if(data.success){
							exportProgressModal.modal('hide');
							if(downFlag){
								$("#downFile").attr("src","${ctx}/fxl/report/exportFinanceReconExcel/" + data.datePath + "/" + data.excelFileName);
							}
						}else{
...
<div class="row">
  <div class="col-md-8"><h3>财务对账管理</h3></div>
...
<div class="row">
  <div class="col-md-2"><h4>财务对账列表</h4></div>
...
<div id="selectDiv" class="text-center">
	<form id="searchForm" action="${ctx}/fxl/report/getFinanceReconList" method="post" class="form-inline" role="form">
