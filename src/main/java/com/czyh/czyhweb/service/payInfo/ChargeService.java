package com.czyh.czyhweb.service.payInfo;
...
import com.czyh.czyhweb.util.pingPay.bean.ChargeParam;
import com.czyh.czyhweb.util.pingPay.util.PingPPUtil;
...
public class ChargeService {
	...
	public Refund createRefund(TPayInfo tPayInfo) {
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("amount", tPayInfo.getFpayAmount().multiply(new BigDecimal(100)).intValue()); // 订单总金额,
		params.put("description", "退款");

		Refund refund = null;
		try {
			refund = Refund.create(tPayInfo.getFpayId(), params);
		} catch (AuthenticationException e) {
			e.printStackTrace();
		} catch (InvalidRequestException e) {
			e.printStackTrace();
