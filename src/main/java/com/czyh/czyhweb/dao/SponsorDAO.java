...
public interface SponsorDAO extends JpaRepository<TSponsor, String>, JpaSpecificationExecutor<TSponsor> {
	...
	@Modifying
	@Query("update TSponsor t set t.fbalance = t.fbalance + ?2 where t.id = ?1")
	void updateBalance(String id, BigDecimal total);
	
	
	@Query("select t from TSponsor t where t.fstatus = 1")
	List<TSponsor> findByStatus();
}